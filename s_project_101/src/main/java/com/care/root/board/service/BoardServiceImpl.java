...
public class BoardServiceImpl implements BoardService{
	...
	public String writeSave(MultipartHttpServletRequest mul, 
		...
		//BoardFileService bfs = new BoardFileServiceImpl();도 가능하지만
		//new로 만들면 호출될 때마다 새로 생성, 삭제가 반복됨.(로딩시간 더 걸릴 수 있음)
		//@Autowired BoardFileService bfs; > 어노테이션 쓸 때. @service로 빈생성 해야함

		if(file.getSize() != 0) {
			//이미지 있을경우 처리
			dto.setImageFileName(bfs.saveFile(file));//파일 이름(날짜 포함) 세팅
		}else {
		...
		//mapper.writeSave(dto) : 성공실패로 0/1 들어옴
	...
	private void upHit(int writeNo) {//게시글 매기기
	...
	public String boardDelete(int writeNo,String imageFileName, 
		...
		String message = null;
		if(result == 1) { bfs.deleteImage(imageFileName); 
	...
	public void getData(int writeNo, Model model) {
		model.addAttribute("personalData", mapper.contentView(writeNo));
		//이름을 personalDatadl으로 만들어 가져오겠다
	}
	public String modify(MultipartHttpServletRequest mul, HttpServletRequest request) {
		...
		if(file.getSize() != 0 ) {
			...
			//bfs.saveFile(file):연월일 붙여 파일명을 리턴으로 돌려주고 파일 저장
			//setImageFileName : 새로운 파일명으로 바꿔줌
